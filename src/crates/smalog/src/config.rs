...
pub struct MqttConfig {
    ...
    /// `{serial}` are expanded (e.g. `smalog/{serial}`). The shared bridge
    /// availability topic is derived from the prefix before `{serial}`.
    #[serde(default = "default_mqtt_base_topic")]
    pub base_topic: String,
    /// Also publish Home Assistant MQTT-Discovery configs. When enabled,
    /// the full metric registry is published (the `data` list is ignored).
    #[serde(default)]
    pub homeassistant: bool,
    /// Home Assistant discovery prefix (HA default `homeassistant`).
...
/// CSV export (SBFspot `CSV_*` / `OutputPath` keys). Disabled by default;
/// the database remains the primary store.
#[derive(Debug, Clone, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CsvConfig {
    /// Master switch (SBFspot `CSV_Export`).
    #[serde(default)]
    pub enabled: bool,
    /// Directory for spot/day/month CSVs. `%Y`/`%m`/`%d` (and other
    /// strftime specifiers) are expanded against the day being written
    /// (SBFspot `OutputPath`).
    ...
    /// Write the SMA 8-line extended header block (SBFspot
    /// `CSV_ExtendedHeader`).
    ...
    #[serde(default = "default_true")]
    pub header: bool,
    /// Pad the spot/day file with zero-power rows for 00:00–23:55
    /// (SBFspot `CSV_SaveZeroPower`).
    #[serde(default)]
    pub save_zero_power: bool,
    /// Field separator (SBFspot `CSV_Delimiter`).
    #[serde(default = "default_csv_delimiter")]
    pub delimiter: String,
    /// Decimal separator for numbers (SBFspot `DecimalPoint`).
    #[serde(default = "default_decimal_point")]
    pub decimal_point: String,
    /// strftime pattern for timestamp columns (SBFspot `DateTimeFormat`).
    #[serde(default = "default_csv_datetime_format")]
    pub datetime_format: String,
    /// strftime pattern for the month-CSV date column (SBFspot
    /// `DateFormat`).
    #[serde(default = "default_csv_date_format")]
    pub date_format: String,
    /// Timestamp source for spot rows (SBFspot `CSV_Spot_TimeSource`):
    /// the inverter's own clock, or this host's clock.
    #[serde(default)]
...
/// Which clock stamps CSV spot rows (SBFspot `CSV_Spot_TimeSource`).
#[derive(Debug, Clone, Copy, Deserialize, Default, PartialEq, Eq)]
#[serde(rename_all = "lowercase")]
pub enum SpotTimeSource {
    /// The inverter's reported time (SBFspot default).
    #[default]
    Inverter,
