# Application Log Buffer Specification
...
without access to stdout, the container log or the systemd journal. The
records are kept in process memory only: a log line is cheap to emit and
expensive to store, and persisting one would put a database write behind every
log call.
