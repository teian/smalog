...
| `calc_missing_spot` | bool | `false` | SBFspot `CalculateMissingSpotValues`: derive missing Pdc/Pac values from voltage × current when the inverter reports zero. **Set this if a tracker shows 0 W beside a real DC voltage and current** — older inverters such as the SB 2500 never send `DcMsWatt`, and their string power (and with it the efficiency) stays empty without it. The service logs a hint when it sees that combination. |
| `show_temperature` | bool | `true` | Carry inverter temperature in the day view. Inverters without a sensor answer "LRI not available", so their column stays empty; set `false` to drop it from the payload and the dashboard entirely. |
